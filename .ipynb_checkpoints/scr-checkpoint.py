...
def SCR(w, loss, gradient, Hv=None, hessian=None, X=None, Y=None, opt=None,**kwargs):
    ...
    **kwargs : dict, optional 
    ...
    grad = gradient(w, X, Y, lambd, alpha)  
    ...
    ## our modification
    accuracy_collector = []
    accuracy = Experiment.compute_accuracy(X, Y, w)
    accuracy_collector.append(accuracy)
    print(f'Accuracy epoch 0: {accuracy}')
    
    
    
    _loss = loss(w, X, Y, lambd, alpha) 
    ...
    for i in range(n_iterations):
        ...
        if gradient_sampling_flag==True or successful_flag==True:
            grad = gradient(w, _X2, _Y2, lambd, alpha, delta=1e-3)
            grad_norm =np.linalg.norm(grad)  
        ...
        (s,lambda_k) = solve_ARC_subproblem(grad,Hv,hessian,sigma, _X, _Y, w, successful_flag, lambda_k,subproblem_solver,
            exact_tol,krylov_tol,solve_each_i_th_krylov_space,keep_Q_matrix_in_memory, lambd, alpha)
        sn=np.linalg.norm(s)

        #### III: Regularization Update #####
        previous_f = loss(w, X, Y, lambd, alpha)
        current_f = loss(w + s, X, Y, lambd, alpha)

        function_decrease = previous_f - current_f
        Hs=Hv(w, _X, _Y, s, lambd, alpha)
        ...
        ## Our modification
        accuracy = Experiment.compute_accuracy(X, Y, w)
        #print('Accuracy:', accuracy, 'Size:', accuracy.shape)
        accuracy_collector.append(accuracy)
    ...
    return w, accuracy_collector, loss_collector, timings_collector, samples_collector

def solve_ARC_subproblem(grad,Hv,hessian, sigma, X, Y, w, successful_flag,lambda_k,subproblem_solver,exact_tol,krylov_tol,solve_each_i_th_krylov_space, keep_Q_matrix_in_memory, lambd, alpha):
    

    if subproblem_solver == 'cauchy_point':
        #min m(-a*grad) leads to finding the root of a quadratic polynominal
        
        Hg=Hv(w, X, Y, grad, lambd, alpha)
        gHg=np.dot(grad,Hg)
        ...
        H =hessian(w,X, Y, lambd, alpha, delta=1e-3)
        ...
        while True:
            ...
            Hq=Hv(w, X, Y, q, lambd, alpha) #matrix free            
            delta_k=np.dot(q,Hq)
        ...
        for j in range (0,k+1):
            if keep_Q_matrix_in_memory:
                ...
                q_re=y/gamma[j]
                Q[:,j]=q_re
                Hq=Hv(w, X, Y, q_re, lambd, alpha) #matrix free
